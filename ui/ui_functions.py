...
def path_derivation_func(path_string: str, ui):
    ...
    if derivation_path == ['']:
        derivation_path = [0]
    print('CHECK DEV PATH IN UI', derivation_path)
...
def seed_button(ui):
    ...
    if address_type in ['bip44', 'bip49', 'bip84']:
        ...
        address_type = 'p2pkh' if address_type == 'bip44' else 'p2wpkh-p2sh' if address_type == 'bip49' else 'p2wpkh'
        hardened_items = [True, True, True, False]
    elif address_type == 'bip141':
        address_type = 'p2wpkh-p2sh'
        hardened_items[-1] = False

    result = seed_to_master(seed, passphrase, derivation_path, hardened_items, total_addresses, address_type, testnet)
